...
export default class ComintBuffer {
  ...
  //
  sendCurrentCommand() {
    this.sendCommand(this.getCurrentCommand());
  }
  ...
  bashAutoComplete(prefix) {
    ...
    this[_term].send("\x1b*"); // Meta-*, default binding for bash's `insert-completions` command
    this[_term].send("\x16\x04!!!"); // C-v C-d !!! -- print a literal ^D!!! at the end. we'll look for that to be our signal that output is over.
    return promise;
  }
